...
import DepositErc20 from "./Lagom/DepositErc20";
...
const TabsBody = ({ loading }: { loading: boolean }) => {
  ...
  const [isLagomOpen, setIsLagomOpen] = React.useState(true);
      ...
      >
        ...
        >
          ...
          >
            ...
            <ListItemText primary="Lagom" sx={{ opacity: open ? 1 : 0 }} />
            {isLagomOpen ? <ExpandLess /> : <ExpandMore />}
          </ListItemButton>
          <Collapse in={isLagomOpen} timeout="auto" unmountOnExit>
            <List component="div" disablePadding>
              {LagomList.map((ele, index) => (
                <ListItem
                  key={ele.name}
                  disablePadding
                  sx={{ display: "block" }}
